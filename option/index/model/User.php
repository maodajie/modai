...
class User extends Base
{
    ...
    public function register($param)
    {
        ...
        if (0 == $config['user']['status'] || 0 == $config['user']['reg_open']) {
        ...
        if (empty($param['user_openid_qq']) && empty($param['user_openid_weixin']) && !captcha_check($data['verify']) && 1 == $config['user']['reg_verify']) {
        ...
        $fields['user_portrait'] = (string) $param['user_portrait'];
		$fields['user_nick_name'] = (string) $param['user_nick_name'];
        if (empty($param['user_openid_qq']) && empty($param['user_openid_weixin']) && '1' == $config['user']['reg_phone_sms']) {
            $param['type'] = 3;
        ...
        } elseif (empty($param['user_openid_qq']) && empty($param['user_openid_weixin']) && '1' == $config['user']['reg_email_sms']) {
            $param['type'] = 3;
            $res = $this->check_msg($param);
            if ($res['code'] > 1) {
                return ['code' => $res['code'], 'msg' => $res['msg']];
    ...
    public function login($param)
    {
        ...
        if (empty($data['openid'])) {
            ...
            if (1 == $GLOBALS['config']['user']['login_verify'] && !captcha_check($data['verify'])) {
                return ['code' => 1002, 'msg' => lang('verify_err')];
            }
    ...
    public function popedom($type_id, $popedom, $group_id = 1)
    {
        ...
        if (false !== strpos(',' . $group_info['group_type'], ',' . $type_id . ',') && false !== !empty($group_info['group_popedom'][$type_id][$popedom])) {
            return true;
        }
    ...
    public function check_msg($param)
    {
        ...
        if ('email' == $param['ac'] && intval($GLOBALS['config']['email']['time']) > 0) {
    ...
    {
        //三级分销
        if ($fee_points > 0 && '1' == $GLOBALS['config']['user']['reward_status']) {
